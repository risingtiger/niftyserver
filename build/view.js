...
const HandlePath = (viewpath, static_prefix, nodeenv, json_configs)=>new Promise(async (resolve, reject)=>{
        const promises = [];
        let r = {};
	...
	`;
        ...
        resolve({
            returnstr: htmlstr,
            viewname: this_lazyload.name
        });
    });
const getlazyload = (path, all_lazyloads)=>{
...
const getview_base_path = (lazyload)=>{
    let view_file_path = "";
...
const get_all_lazyload_dependencies = (all_lazyloads, lazyload, static_prefix, paths_list)=>{
    debugger;
    for (const dep of lazyload.dependencies){
        const dep_lazyload = all_lazyloads.find((v)=>v.name == dep.name);
        let resofpath = "views/" // default
        ;
        switch(dep_lazyload.type){
            case "view":
                resofpath = "lazy/views/" + dep_lazyload.name + "/" + dep_lazyload.name + ".js";
                break;
            case "component":
                resofpath = "lazy/components/" + dep_lazyload.name + "/" + dep_lazyload.name + ".js";
                break;
            case "thirdparty":
                resofpath = "thirdparty/" + dep_lazyload.name + ".js";
                break;
        }
