...
function Retrieve(bucket, fluxstr) {
    return new Promise(async (res, _rej)=>{
        let token = bucket === "PWT" ? PWTT : XENT;
...
function setcomplexfluxstr(index, bucket, msr, fields, tags, begin, end, intrv, priors) {
    let calls = [
        {
            range_begin: begin,
            range_end: end,
    ...
    return bodystr;
    function get_priorsstr(priors_str) {
        const callshere = [];
        if (priors_str) {
            ...
            priorssplit.pop();
            let unit_seconds = 0;
            if (unit === "d") unit_seconds = 86400;
            else if (unit === "h") unit_seconds = 3600;
            else if (unit === "m") unit_seconds = 60;
            for(let i = 0; i < priorssplit.length; i++){
                const amount = Number(priorssplit[i]);
                callshere.push({
                    range_begin: begin - amount * unit_seconds,
                    range_end: end - amount * unit_seconds,
                    priors_amount: amount,
                    priors_unit: unit
                });
            }
        }
...
function callserver(fluxstr, token) {
    return new Promise(async (res, _rej)=>{
        const obj = {
            method: "POST",
            headers: {
...
function parseit_for_series(queries_length, data_str, fields, tags, priors) {
    const queries_list = [];
    let last_tag_name = "";
    for(let i = 0; i < queries_length; i++){
        queries_list.push([]);
        const q = queries_list[i];
        fields[i].split(',').forEach((f)=>{
            tags[i].split(',').forEach((t)=>{
                ...
                if (priors_split.length > 1) {
                    ...
                    for(let j = 0; j < priors_split.length - 1; j++){
                        q.push({
                            field: f,
                            tag: {
                                name: tagobj[0],
...
function parseit_for_points(data_str) {
    const points_list = [];
    data_str.split("\n").forEach((data_row)=>{
        if (data_row.length < 20) return;
        const c = data_row.split(",");
...
function get_fieldstr(fields_str) {
