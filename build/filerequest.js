...
function runit(fileurl, res, env, static_prefix, nocache = true) {
    return new Promise(async (resolve, _reject)=>{
        if (nocache) res.set('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate, max-age=0');
...
async function js(absolute_path, jspath, jsextension, env, res) {
...
async function css(absolute_path, csspath, cssextension, env, res) {
...
const fleshitout = (absolute_path, path_without_extension, includemaincss = false)=>new Promise(async (resolve, _reject)=>{
        ...
        let css_replace = "";
        let [htmlstr, jsstr] = await Promise.all([
            htmlpromise,
            jspromise
        ]);
...
const get_view_parts_str = (absolute_path, path_without_extension)=>new Promise(async (resolve, _reject)=>{
        ...
        if (pstat) {
            const dir_entries = await fsp.readdir(parts_dir_fs_path, {
                withFileTypes: true
            });
